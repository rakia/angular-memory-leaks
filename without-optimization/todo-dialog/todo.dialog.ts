...
export class TodoDialog implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.todoService.getTodos() // .pipe(takeUntil(this.destroy$))
      .subscribe(todos => {
      this.dependencies        = todos;
      this.filteredTodoList = this.dependencies;
    });
    this.todoService.getTypes() // .pipe(takeUntil(this.destroy$))
  ...
  createTodoForm(todo: TODO, index: number): FormGroup {
    ...
    formGroup.valueChanges.subscribe(selectedType => {
      console.log('form value changes');
    });
    // this.typeChangesUnsubscriptions[index] =
    formGroup.get('type').valueChanges.subscribe(selectedType => {
        console.log('type value changes');
